...
应选择能够覆盖实际判断要求的最低档位。动作、规则和验收都已明确时使用 Fast；结果明确但仍需
调查证据并做有限的分类、保留、优先级或实现选择时使用 Normal；需要定义目标或规则、调和模糊
冲突的要求、设计新的系统结构、寻找未知的系统级根因，或处理特别复杂的逻辑时才使用 Deep。
任务名称本身不决定档位：按明确清单移动文档是 Fast，从混杂资料中判断保留、合并和删除则是
Normal。

Fast 的实用门槛是：派发前已经明确具体结果、限定的方法或范围，以及能够证明完成的可观察证据。
前期规划即使很困难，形成精确工作单后的实现仍可使用 Fast；如果 Worker 执行时还必须发现其中
一个答案，则使用 Normal。不要仅为换档重启已经有效工作的任务并重复发送上下文。
