...
Profile 逐步混入 Fast 或 Normal 的第一阶段；二级把 Fast/Normal 的后备维持在配置上限，并把
自动 Normal 的来源暂时换成第一个可用的 Fast 来源池。Deep 和显式指定 Profile 永不改变。Agent Plan 预设的
一级/二级基准为 38%/25%，DeepSeek 上限为 30%。这些数值是动态护栏：工作池按近期真实消耗
拟合，若下一次补额很近且能显著恢复总池，两级生效阈值最多自动下调 30%；额度未知时不虚构补额。
