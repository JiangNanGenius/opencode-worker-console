...
可选的 `quota_spillover` 规则补上了“套餐健康”和“套餐完全耗尽”之间的空档。当状态为
`fast_preferred` 时，它可以把后段按量 Profile 提前混入 Fast 或 Normal 的第一阶段；比例在
续航临界线处为 0%，随着可用套餐中较好的续航继续下降而逐步升高，最多达到设置的上限。
额度未知时不触发，显式指定 Profile 不受影响，也只作用于勾选的档位。Agent Plan 预设默认
让 DeepSeek 最多承担 30% 的 Fast/Normal 请求，Deep 档仍保留强模型阶梯。
