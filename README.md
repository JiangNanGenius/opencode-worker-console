...
- **Tasks:** delegate coherent bounded outcomes to general-purpose agents, including SSH/remote administration, deployment, CLI/API workflows, research, writing, data processing, implementation and verification. Remote-only changes can use operational targets without a dummy local file scope or Git repository. These examples are not a capability allowlist; select work by task fit, available tools and the user's authorization. Inspect evidence and cancel safely.
...
- **Billing recovery:** Kimi monthly exhaustion is an advisory shared across its profiles: automatic selection avoids it, while an explicit profile choice can retry. A verified successful response clears the flag. Configure a monthly billing day, time and time zone to release the previous cycle automatically. Endpoint-confirmed zero allowance remains unavailable even for explicit retries. Ordinary positive usage telemetry alone does not prove monthly recovery. The coordinator reviews partial work before continuing; the bridge does not replay dispatched operations.
