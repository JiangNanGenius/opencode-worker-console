...
def display_endurance(config, pool, guidance, routing_status, control=None, now=None):
    ...
    if raw is None:
        return {'raw_hours': None, 'adjusted_hours': None, 'multiplier': 1.0,
                'provider_multipliers': {}, 'conservation_level': level, 'basis': 'collecting'}
    if level not in (1, 2):
        return {'raw_hours': raw, 'adjusted_hours': raw, 'multiplier': 1.0,
                'provider_multipliers': {}, 'conservation_level': level, 'basis': 'observed_burn'}
    ...
    spill_share = max(0.0, min(0.7, spill_share))
    ...
    if level == 2:
        ...
        # Automatic Normal work now uses Ark Auto, while fallback-routed work
        # leaves Ark entirely. Keep 40% of observed Ark pressure unadjusted for
        # Deep, pinned and mixed workloads, then apply the configured Auto versus
        # Evolving coefficient to the remaining 60%. This is deliberately bounded.
        efficiency = max(0.1, min(1.0, values['ark_auto_afp_per_m'] /
                                  values['ark_evolving_afp_per_m']))
        adjusted_burn = 0.40 + 0.60 * (1.0 - spill_share) * efficiency
        ark_full = min(3.0, 1.0 / max(0.34, adjusted_burn))
        provider_multipliers['volcengine-agent-plan'] = round(
            1.0 + (ark_full - 1.0) * blend, 3)
