...
- **任务协作**：在任务下方原位展开详情，直接查看实时活动及输入、输出、推理、缓存 Token。按当前筛选汇总已记录的 Worker 用量，不含主 Agent 用量，也不代表费用。清理已结束任务前先保存精简的 Token、成本与路由账本，再删除关联 OpenCode 对话和可释放证据；已整合或确认无改动的隔离工作树会释放，未整合改动保留待复核。
- **会话管理**：查找、新建、改名、分叉、归档/恢复、绑定工作区、删除、跳转 OpenCode。分叉本身不发送提示词。
- **模型、额度与经济性**：可视化配置单供应商、顺序主备、固定比例池或主动开启额度自适应，并读取 DeepSeek 余额、Kimi 套餐窗口和方舟 AFP 窗口。每个服务商可选择实时额度、手动刷新窗口、金额低余额阈值，或不参与动态判断；全局的刷新感知续航阈值也可修改。提前分流规则可在套餐全部耗尽前，逐步把限定比例的 Fast/Normal 请求交给后段按量模型；周额度低位保护则保留最后的 Kimi 额度。确认的额度终止和模型端 429 可在同一会话内切换下一路由继续。控台增加总工作池卡片、可视化剩余额度与刷新进度，并同时显示北京时间与本机时间、当前动态比例和可配置的统一成本尺（默认 `1 AFP-equivalent = ¥0.002`）；AFP-equivalent 不是 Kimi 官方额度。
...
- **数据统计**：独立统计页合并当前任务和清理后精简账本，展示近 30 天活动、按模型/档位/状态/Profile 的 Token、兜底次数及近期额度/余额采样。
