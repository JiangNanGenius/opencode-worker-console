...
- **任务协作**：在任务下方原位展开详情，先立即显示缓存元数据，再用有限超时读取完整会话，避免一直卡在“正在读取详情”。可查看实时活动及输入、输出、推理、缓存 Token；较长消息点击“展开全文”才按需读取完整且已脱敏的内容。按当前筛选汇总已记录的 Worker 用量，不含主 Agent 用量，也不代表费用。清理已结束任务前先保存精简的 Token、成本与路由账本，再删除关联 OpenCode 对话和可释放证据；`needs_attention` 空闲满 24 小时，或同一 Codex 会话下已有更新任务时，也可回收。已整合或确认无改动的隔离工作树会释放，未整合改动保留待复核。
- **会话管理**：查找、新建、改名、分叉、归档/恢复、绑定工作区、删除、跳转 OpenCode。分叉本身不发送提示词。
- **模型、额度与经济性**：可视化配置单供应商、顺序主备、固定比例池或主动开启额度自适应，并读取 DeepSeek 余额、Kimi 套餐窗口和方舟 AFP 窗口。每个服务商可选择实时额度、手动刷新窗口、金额低余额阈值，或不参与动态判断。两级降载会在可持续范围内尽量保留模型能力：一级逐步把 Fast/Normal 的一部分交给后段后备；二级按配置的后备上限维持 Fast/Normal 的稳定比例，并让自动 Normal 暂时使用 Fast 来源池，Deep 保持不变。38%/25% 只是基准护栏；系统按近期真实消耗和补额后的预计总池动态下调生效阈值，不用固定原始百分比硬切。周额度低位保护保留最后的 Kimi 额度，确认的额度终止和模型端 429 可在同一会话内切换下一路由继续；额度降载期间，符合条件的自动 Fast/Normal 长任务还能在当前回合结束后排入同会话换模型续接，不会中断当前回合，Deep 和显式指定模型保持不变。控台按各服务商的实际消耗速度拟合续航：总条彩色部分表示当前剩余，右侧留白表示已消耗，AFP 数值和套餐价格不参与加权。每个供应商只在最紧张的额度窗口显示续航倍率，因为真正生效的是这个短板。Kimi 与方舟保留各自独立的刷新时间；最先到来的刷新及刷新后的总池预测单独显示，不会提前混入当前余额。DeepSeek 续航会把余额长期变化和输入、缓存命中、输出 Token 的官方价格结合，默认 Flash/Pro 高峰价与闲时倍率也可在设置中修改；成本换算只留在统计和设置中，不再伪装成供应商总额度。
...
- **推送与凭据引用**：Bark 默认只推额度关键节点：降载级别变化、供应商耗尽与恢复。逐任务换模推送为可选项，默认关闭。推送默认使用 Worker Desk 四宫格图标和可配置的 GitHub HTTPS 地址；Bark 下载成功后会缓存，清空即可沿用默认图标。Bark 地址和方舟控制面 AK/SK 都只登记环境变量名或仅当前用户可读的本机文件引用；页面与接口不会返回密钥原文或源路径。可在 **模型与调度** 中设置。[凭据说明](references/credentials.md)。
