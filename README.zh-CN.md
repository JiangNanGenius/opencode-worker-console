...
- **模型、额度与经济性**：可视化配置单供应商、顺序主备、固定比例池或主动开启额度自适应，并读取 DeepSeek 余额、Kimi 套餐窗口和方舟 AFP 窗口。可配置的周额度低位保护会把最后的 Kimi 额度留给有限数量的原厂 K3；确认的额度终止和模型端 429 可在同一会话内切换下一路由继续。控台显示当前动态比例和可配置的统一成本尺（默认 `1 AFP-equivalent = ¥0.002`）；AFP-equivalent 不是 Kimi 官方额度。
