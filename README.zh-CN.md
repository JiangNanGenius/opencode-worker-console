...
当 Kimi 健康而方舟仍严重紧张时，普通档可接近 Kimi 70% / 方舟 0% / DeepSeek 30%；兜底只连续替代紧张方舟原有的份额，
让方舟优先保留给 Fast。方舟按重置时间计算的续航改善后会重新进入 Normal。
...
额度保护使用两条连续曲线。一级随着套餐续航下降，逐步让后段按量模型承担 Fast/Normal；Fast 按自己的来源套餐续航计算，不会被刚恢复的 Normal/Deep 套餐掩盖。二级下降更快，并暂时让自动 Normal 使用 Fast 来源池。Deep 和显式指定模型保持不变。DeepSeek 余额高于保护线时，二级高压上限为 75%：闲时更快到达上限，峰时只有套餐续航严重不足才从一级上限逐步升到 75%。周末始终按闲时处理，工作日法定节假日通过可配置订阅识别并在本机缓存。详见[路由与套餐利用率](references/routing.zh-CN.md)。
...
- 桥接层和网页控台可查询、新增、编辑和删除项目长期记忆；可选的 `opencode-mem` 在本机保存向量库，并可连接 OpenAI 兼容的嵌入接口。
