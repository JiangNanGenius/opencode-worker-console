...
- **推送与凭据引用**：Bark 默认只推额度关键节点：降载级别变化、供应商耗尽与恢复。逐任务换模推送为可选项，默认关闭。Bark 地址和方舟控制面 AK/SK 都只登记环境变量名或仅当前用户可读的本机文件引用；页面与接口不会返回密钥原文或源路径。可在 **模型与调度** 中设置。[凭据说明](references/credentials.md)。
