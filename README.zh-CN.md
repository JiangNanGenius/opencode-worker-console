...
额度保护使用两条连续曲线。一级随着套餐续航下降，逐步让后段按量模型承担 Fast/Normal；二级下降更快，并暂时让自动 Normal 使用 Fast 来源池。Deep 和显式指定模型保持不变。DeepSeek 闲时且余额高于配置下限时，二级上限可提高到 50%；高峰或低余额时回到一级上限。周末始终按闲时处理，工作日法定节假日通过可配置订阅识别并在本机缓存。详见[路由与套餐利用率](references/routing.zh-CN.md)。
