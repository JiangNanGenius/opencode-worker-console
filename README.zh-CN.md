...
- **模型、额度与经济性**：可视化配置单供应商、顺序主备、固定比例池或主动开启额度自适应，并读取 DeepSeek 余额、Kimi 套餐窗口和方舟 AFP 窗口。每个服务商可选择实时额度、手动刷新窗口、金额低余额阈值，或不参与动态判断；全局的刷新感知续航阈值也可修改。周额度低位保护只有在 Kimi 剩余额度预计撑不到刷新时才生效；确认的额度终止和模型端 429 可在同一会话内切换下一路由继续。控台增加总工作池卡片、可视化剩余额度与刷新进度，并同时显示北京时间与本机时间、当前动态比例和可配置的统一成本尺（默认 `1 AFP-equivalent = ¥0.002`）；AFP-equivalent 不是 Kimi 官方额度。
