...
- **模型、额度与经济性**：可视化配置单供应商、顺序主备、固定比例池或主动开启额度自适应，并读取 DeepSeek 余额、Kimi 套餐窗口和方舟 AFP 窗口。每个服务商可选择实时额度、手动刷新窗口、金额低余额阈值，或不参与动态判断；全局的刷新感知续航阈值也可修改。提前分流规则可在套餐全部耗尽前，逐步把限定比例的 Fast/Normal 请求交给后段按量模型；周额度低位保护则保留最后的 Kimi 额度。确认的额度终止和模型端 429 可在同一会话内切换下一路由继续。控台按各服务商的实际消耗速度拟合续航：总条彩色部分表示当前剩余，右侧留白表示已消耗，AFP 数值和套餐价格不参与加权。Kimi 与方舟保留各自独立的刷新时间；最先到来的刷新及刷新后的总池预测单独显示，不会提前混入当前余额。DeepSeek 续航会把余额长期变化和输入、缓存命中、输出 Token 的官方价格结合，默认 Flash/Pro 高峰价与闲时倍率也可在设置中修改；成本换算只留在统计和设置中，不再伪装成供应商总额度。
