...
(function (global) {
  ...
  var MESSAGES = {
    en: {
      ...
      "routing.policyHint": "Stages run left to right; weights share task admissions within a stage. Explicit profiles stay pinned.",
    ...
    'zh-CN': {
      ...
      "routing.policyHint": "从左到右选择可用阶段；同一阶段按权重分配任务。显式选择 Profile 会固定模型。",
    ...
    'zh-TW': {
      ...
      "routing.policyHint": "從左到右選擇可用階段；同一階段依權重分配任務。明確選擇 Profile 會固定模型。",
    ...
    ja: {
      ...
      "routing.policyHint": "左から利用可能な段階を選び、段階内では重みに応じてタスクを配分します。明示したプロファイルは固定されます。",
    ...
    ko: {
      ...
      "routing.policyHint": "왼쪽부터 사용 가능한 단계를 선택하고 단계 안에서 가중치에 따라 작업을 배분합니다. 명시한 프로필은 유지됩니다.",
