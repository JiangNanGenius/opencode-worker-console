...
function quotaWindows(windows,showValues=false){return windows.length?windows.map(w=>{const p=w.remaining_percent,name=windowName(w),runway=windowRunway(w),threshold=(data.tier_guidance?.runway_threshold_percent??75)/100;return `<div><div class="window-heading"><span>${esc(name)}</span><strong><small>${tr('quota.remaining')}</small>${p==null?'—':(I18n?I18n.number(Math.round(p)):Math.round(p))+'%'}</strong></div><progress class="quota-bar ${runway!=null&&runway<threshold?'low':''}" aria-label="${esc(tr('quota.remainingAria',{name}))}" max="100" ${p==null?'':`value="${Math.max(0,Math.min(100,p))}"`}></progress>${showValues&&w.remaining!=null?`<div class="window-value">${esc(number(Math.round(w.remaining)))} / ${esc(number(Math.round(w.limit)))} AFP</div>`:''}${runway==null?'':`<div class="window-value">${esc(tr('quota.runway',{value:fixed2(runway)}))}</div>`}<div class="window-reset">${esc(resetTime(w.resets_at))}</div></div>`}).join(''):'<p class="muted">'+tr('quota.unknown')+'</p>';}
...
$('clear-completed-tasks').addEventListener('click',()=>{const n=data.tasks.filter(t=>['completed','needs_attention'].includes(t.status)).length;if(!n)return;openAction(tr('dialog.clearCompleted'),`<p>${esc(tr('dialog.clearCompletedHint',{n}))}</p>`,async()=>{await mutate('/console-api/tasks/manage',{action:'clear_finished'});selectedTaskIds.clear();await refresh();});});
