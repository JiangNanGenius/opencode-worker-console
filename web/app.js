...
function windowRunway(w){const p=Number(w?.remaining_percent),d=resetDate(w?.resets_at),duration=Number(w?.duration_minutes);if(!Number.isFinite(p)||!d||!Number.isFinite(duration)||duration<=0||d.getTime()<=Date.now())return null;const fraction=Math.max(0,Math.min(1,(d.getTime()-Date.now())/(duration*60000)));return fraction>0?Math.max(0,Math.min(4,p/100/fraction)):null;}
...
function quotaTrack(p,expected,label){const value=Number.isFinite(Number(p))?Math.max(0,Math.min(100,Number(p))):0;const marker=Number.isFinite(expected)?`<line class="quota-pace-marker" x1="${expected}" x2="${expected}" y1="0" y2="8"/>`:'';return `<svg class="quota-track" viewBox="0 0 100 8" role="img" aria-label="${esc(label)}"><rect class="quota-track-bg" x="0" y="1" width="100" height="6" rx="3"/><rect class="quota-track-fill" x="0" y="1" width="${value}" height="6" rx="3"/>${marker}</svg>`;}
...
function taskCleanupResult(result){const bytes=Number(result?.freed_bytes)||0,size=bytes>=2**30?fixed2(bytes/2**30)+' GiB':fixed2(bytes/2**20)+' MiB';$('task-cleanup-message').textContent=tr('tasks.cleanupResult',{deleted:result?.deleted||0,sessions:result?.sessions_deleted||0,worktrees:result?.worktrees_released||0,retained:result?.retained_for_review?.length||0,size});}
...
function quotaWindows(windows,showValues=false){return windows.length?windows.map(w=>{const p=w.remaining_percent,name=windowName(w),runway=windowRunway(w),expected=expectedRemaining(w),pace=quotaPace(w),display=p==null?'—':(I18n?I18n.number(Math.round(p)):Math.round(p))+'%';const delta=Number.isFinite(Number(p))&&Number.isFinite(expected)?Number(p)-expected:null;const comparison=delta==null?'':tr(delta>=0?'quota.paceAhead':'quota.paceBehind',{value:I18n?I18n.number(Math.abs(Math.round(delta))):Math.abs(Math.round(delta))});return `<article class="quota-window ${pace[0]}"><div class="quota-window-main"><svg class="quota-ring" viewBox="0 0 42 42" role="img" aria-label="${esc(tr('quota.remainingAria',{name}))}"><circle class="quota-ring-bg" cx="21" cy="21" r="15.9" pathLength="100"/><circle class="quota-ring-value" cx="21" cy="21" r="15.9" pathLength="100" stroke-dasharray="${Number.isFinite(Number(p))?Math.max(0,Math.min(100,Number(p))):0} 100"/><text x="21" y="22.5">${esc(display)}</text></svg><div class="quota-window-copy"><div class="window-heading"><span>${esc(name)}</span><strong class="quota-pace-label">${esc(pace[1])}</strong></div>${quotaTrack(p,expected,tr('quota.remainingAria',{name}))}<div class="quota-pace-copy">${esc(comparison||tr('quota.expectedRemaining'))}${runway==null?'':`<span class="quota-runway">${esc(fixed2(runway))}×</span>`}</div>${showValues&&w.remaining!=null?`<div class="window-value">${esc(number(Math.round(w.remaining)))} / ${esc(number(Math.round(w.limit)))} AFP</div>`:''}<div class="window-reset">${esc(resetTime(w.resets_at))}</div></div></div></article>`}).join(''):'<p class="muted">'+tr('quota.unknown')+'</p>';}
...
function renderPool(providers){const list=[];if(providers.has('kimi-for-coding'))list.push(poolProvider('kimi-for-coding','Kimi','senior'));if(providers.has('volcengine-agent-plan'))list.push(poolProvider('volcengine-agent-plan',tr('quota.arkShort'),'deep'));const block=document.querySelector('.pool-block');block.hidden=!list.length;if(!list.length)return;const available=list.filter(x=>x.available).length;$('pool-state').textContent=available===list.length?tr('quota.poolReady'):available?tr('quota.poolReduced'):tr('quota.poolUnavailable');const ratio=list.length?available/list.length*100:0;setHTML($('pool-summary'),`<svg class="pool-ring" viewBox="0 0 42 42" role="img" aria-label="${esc(tr('quota.poolAvailable',{available,total:list.length}))}"><circle class="quota-ring-bg" cx="21" cy="21" r="15.9" pathLength="100"/><circle class="quota-ring-value" cx="21" cy="21" r="15.9" pathLength="100" stroke-dasharray="${ratio} 100"/><text x="21" y="20">${available}/${list.length}</text><text class="pool-ring-caption" x="21" y="26">${esc(tr('quota.plans'))}</text></svg><div><strong>${esc(tr('quota.poolAvailable',{available,total:list.length}))}</strong><span>${esc(tr('quota.poolHint'))}</span></div>`);setHTML($('pool-providers'),list.map(x=>`<div class="pool-provider"><div><span><i class="model-dot ${x.dot}"></i>${esc(x.label)}</span><strong>${x.known?Math.round(x.p)+'%':'—'}</strong></div>${x.known?quotaTrack(x.p,expectedRemaining(x.window),tr('quota.poolProviderAria',{provider:x.label})):''}<small>${x.known?esc(windowName(x.window)+' · '+quotaPace(x.window)[1]):esc(tr('quota.unknown'))}</small></div>`).join(''));const resets=list.map(x=>resetDate(x.window?.resets_at)).filter(Boolean).sort((a,b)=>a-b);$('pool-next').textContent=resets.length?tr('quota.poolNext',{time:resetTime(resets[0])}):tr('quota.poolNoReset');}
...
function renderTasks(){
 ...
 $('clear-completed-tasks').disabled=!data.tasks.some(t=>['completed','needs_attention'].includes(t.status));
...
$('task-rows').addEventListener('click',e=>{const remove=e.target.closest('[data-delete-task]');if(remove){const task=data.tasks.find(t=>t.id===remove.dataset.deleteTask);openAction(tr('dialog.deleteTasks'),`<p>${esc(tr('dialog.deleteTasksHint',{n:1}))}</p><p><strong>${esc(task?.title||remove.dataset.deleteTask)}</strong></p>`,async()=>{const result=await mutate('/console-api/tasks/manage',{action:'delete',ids:[remove.dataset.deleteTask]});selectedTaskIds.delete(remove.dataset.deleteTask);await refresh();taskCleanupResult(result);});return;}const b=e.target.closest('[data-detail]');if(b)details(b.dataset.detail);});
$('delete-selected-tasks').addEventListener('click',()=>{const ids=[...selectedTaskIds];if(!ids.length)return;openAction(tr('dialog.deleteTasks'),`<p>${esc(tr('dialog.deleteTasksHint',{n:ids.length}))}</p>`,async()=>{const result=await mutate('/console-api/tasks/manage',{action:'delete',ids});selectedTaskIds.clear();await refresh();taskCleanupResult(result);});});
$('clear-completed-tasks').addEventListener('click',()=>{const n=data.tasks.filter(t=>['completed','needs_attention'].includes(t.status)).length;if(!n)return;openAction(tr('dialog.clearCompleted'),`<p>${esc(tr('dialog.clearCompletedHint',{n}))}</p>`,async()=>{const result=await mutate('/console-api/tasks/manage',{action:'clear_finished'});selectedTaskIds.clear();await refresh();taskCleanupResult(result);});});
