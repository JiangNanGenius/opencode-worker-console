...
function quotaWindows(windows,showValues=false){return windows.length?windows.map(w=>{const p=w.remaining_percent,name=windowName(w);return `<div><div class="window-heading"><span>${esc(name)}</span><strong><small>${tr('quota.remaining')}</small>${p==null?'—':(I18n?I18n.number(Math.round(p)):Math.round(p))+'%'}</strong></div><progress class="quota-bar ${p<20?'low':''}" aria-label="${esc(tr('quota.remainingAria',{name}))}" max="100" ${p==null?'':`value="${Math.max(0,Math.min(100,p))}"`}></progress>${showValues&&w.remaining!=null?`<div class="window-value">${esc(number(Math.round(w.remaining)))} / ${esc(number(Math.round(w.limit)))} AFP</div>`:''}<div class="window-reset">${esc(resetTime(w.resets_at))}</div></div>`}).join(''):'<p class="muted">'+tr('quota.unknown')+'</p>';}
function renderQuota(){const providers=new Set(Object.values(data.profiles||{}).map(p=>p.model.split('/')[0]));const hasArk=providers.has('volcengine-agent-plan');document.querySelector('.quota-block').hidden=!providers.has('deepseek');document.querySelector('.kimi-block').hidden=!providers.has('kimi-for-coding');document.querySelector('.ark-block').hidden=!hasArk;document.querySelector('.quota-section').hidden=!providers.has('deepseek')&&!providers.has('kimi-for-coding')&&!hasArk;const ds=data.quota.deepseek||{}, k=data.quota['kimi-for-coding']||{},ark=data.quota['volcengine-agent-plan']||{};
...
function renderEconomics(){const e=data.economics||{},s=e.settings||{},card=$('economics-card');card.hidden=!Object.keys(e).length;if(card.hidden)return;$('economics-unit').textContent=`1 AFP-eq = ¥${Number(s.afp_cny_per_unit||0).toFixed(3)} · ¥1 = ${number(Math.round(e.afp_eq_per_cny||0))} AFP-eq`;$('economics-kimi').textContent=`¥${number(s.kimi_plan_cny||0)} = ${number(Math.round(e.kimi_plan_afp_eq||0))} AFP-eq`;const a=e.ark_monthly;$('economics-ark').textContent=a?`${number(Math.round(a.used))} / ${number(Math.round(a.limit))} AFP · ¥${fixed2(a.used_cny)}`:tr('quota.unknown');$('economics-formula').textContent=e.baseline_formula||'—';const rows=[];for(const [tier,stages] of Object.entries(data.routing_status||{})){const first=stages?.[0];if(!first)continue;rows.push(`<p><strong>${esc(tr('routing.'+tier))}</strong> · ${Object.entries(first.members||{}).map(([name,m])=>`${esc(data.profiles?.[name]?.label||name)} ${Math.round(100*(m.share||0))}%`).join(' / ')} <span>${esc(first.reason)}</span></p>`);}setHTML($('economics-routing'),rows.join('')||'<p>—</p>');}
